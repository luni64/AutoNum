...
- Applies only to newly created labels (open, redetect, rotate); existing labels are never moved retroactively when the setting changes. Never persisted in metadata — once a label exists its coordinates are indistinguishable from a manually placed one.
- Configured via the reusable `Views/FaceAnchorPicker.xaml` control (a 3x3 grid of dot-style `RadioButton`s, same DependencyProperty pattern as `FontManager`), used in two places with different scope:
  - **Settings window → Erkennung tab**: binds directly to `SettingsManager.FaceLabelAnchor`, the persisted app-wide default (`AppSettings.DefaultFaceLabelAnchor`) applied to every freshly opened image. No redetect action here — same as every other default value, changing it never touches an already-open image.
  - **Label formatting dialog** (`TextFormatDialog` opened for `LabelManager`): binds to `LabelManager.FaceLabelAnchor`, a per-session value scoped to the current image, plus a "Neu Erkennen" button bound to the existing `LabelManager.RedetectFacesCommand` (which already confirms before deleting names). Picking an anchor here only takes effect once "Neu Erkennen" is clicked, and never changes the app-wide default on its own.
  - The only paths that cross from "this image" to "the default" are explicit: the format dialog's **"Als Standard übernehmen"** button, or editing the Erkennung tab directly. `SettingsManager.ApplyCurrentImageFormattingDefaults` (Formatierung tab's "Auf das aktuelle Bild anwenden") deliberately does *not* touch `LabelManager.FaceLabelAnchor` — pushing the default into a live image's working anchor was considered and explicitly rejected as unrequested scope creep.
...
### Numbering Direction
- `LabelManager.Numerate()` is the **only** writer of `MarkerLabel.Number` in the app; everything else (drag, add/remove person, redetect, rotate, row-edit commit) delegates to it. It orders persons by row, then left-to-right by `Label.X`, and assigns 1..n.
- `LabelManager.NumberBottomUp` flips the row walk: `false` (default) starts at the top row, `true` at the bottom row. Within a row the order is always left-to-right. The "unassigned" sentinel for `Row <= 0` flips with the direction (`int.MaxValue` ↔ `int.MinValue`) so row-less labels stay last either way.
- Scope mirrors the other per-image settings, **not** `FaceLabelAnchor`: the direction is persisted per image (optional `NumberBottomUp` field on `AutoNumMetaData_V1`, null in older files → top-down, which is how they were numbered) and seeded from `SettingsManager.NumberBottomUp` on both open paths. The load paths call `SeedNumberBottomUp` rather than the property setter, because the setter renumbers immediately and restored numbers must come from the file, not be recomputed.
- UI: `SettingsManager.NumberBottomUp` (app-wide default) in the settings dialog's **Erkennung** tab; `LabelManager.NumberBottomUp` (current image) as a checkbox in the label `TextFormatDialog`, which also pushes it to the default via "Als Standard übernehmen".
- The names-table renderers stay **direction-agnostic**: `Analyzer.PlacePersonNames` and `PdfReportRenderer` order their row groups by `Min(Number)` inside the group instead of by row index. That is identical to ordering by row index for top-down numbering and automatically correct for bottom-up, so neither renderer (nor `SidecarExportData`, which already sorts by number) needs to know the direction. The names list therefore always reads 1..n.
- Row divider text is the one **display-only** mirror: `NameManager.ResolveDisplayRowNumber` prints `maxRow - row + 1` when numbering bottom-up, so the front row that holds number 1 is also labelled "Reihe 1". Stored `Person.Row` values, `RowBoundary` geometry and row-edit mode keep counting from the top — deliberately, since flipping the stored indices would ripple into row detection, boundaries, metadata and sidecars for no functional gain.
...
- Four tabs:
  ...
  - **Erkennung** (Detection): Face detection enable/disable, row detection enable/disable, the face-relative label anchor (3x3 grid, see Face-Relative Label Anchor above), the default numbering direction (see Numbering Direction above), and a manual "Neu Erkennen" action that re-runs face detection. The detector (YuNet, see External Dependencies) has no user-configurable tuning parameters.
